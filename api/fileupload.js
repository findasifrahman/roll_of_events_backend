...
app.post('/upload', function(req, res){
    ...
    form.on('fileBegin', function (name, file){
      const ext =  path.extname(file.name);
        file.path = __dirname + '/tmpUploads/' +  picid + ext;//file.name;
    });

    form.on('file', function (name, file){
        const ext =  path.extname(file.name);
        console.log('Uploaded ' + file.name);
        return res.json({ success: true,fileid: picid + ext,full: __dirname + '/tmpUploads/' + picid + ext,fullpdf: "/images/pdficon.png", error : ""});          
    });
    form.on('error', (err) => {
...
app.get('/delete', function(req, res){
  console.log("inside file delete--",req.query.type);
  var sp
  if(req.query.type ){
      if(req.query.type == "edit"){
        sp = __dirname + '/uploads/';
      }else{
        sp = __dirname + '/tmpUploads/';
      }
  }else{
    sp = __dirname + '/uploads/';
  }
